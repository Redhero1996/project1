    ...
    <div class="form-group quiz">
        @if (!empty($data))
            ...
            <div class="form-group alert alert-warning" id="score">
                ...
                <span class="alert-link">{{trans('translate.total')}} {{ $data['total'] }}/{{ count($data['topic']) * config('constants.point') }} ( {{ round(($data['total'] / (count($data['topic']) * config('constants.point'))) * config('constants.hundred'), config('constants.two')) }}% )</span>
